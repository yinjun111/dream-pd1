...
#replace counts with TPM
tmm<-counts
inflam_sig  <- rowMedians(scale(t(log2(tmm[rownames(tmm)  %in% c("CD274", "CD8A", "LAG3","STAT1"),]+0.001)))); print("done computing inflamation signatrue")
